...
CLI_SOURCES := $(shell if [ -d "$(ROOT_DIR)/src/cli" ]; then cd "$(ROOT_DIR)" && find src/cli -type f -name '*.c' -print | LC_ALL=C sort; fi)
APP_SOURCES := src/main.c $(CLI_SOURCES)
...
CLI_OBJECTS := $(addprefix $(OBJ_DIR)/,$(CLI_SOURCES:.c=.o))
...
COMMON_COMPILE_FLAGS = \
	$(CPPFLAGS) $(ZLIB_CPPFLAGS) $(FEATURE_CPPFLAGS) $(CFLAGS) \
	$(COVERAGE_COMPILE_FLAGS) \
	$(DEPFILE_COMPILE_FLAGS) $(MJ_COMPILE_FLAGS)
COMMON_LINK_FLAGS = $(COVERAGE_LINK_FLAGS) $(LDFLAGS)
COMMON_LDLIBS = $(ZLIB_LDLIBS) $(LDLIBS)

# Emit one #define line per saved configuration variable.
CONFIG_INFO_DEFINES := \
	$(foreach var,PROFILE PREFIX CC CPPFLAGS CFLAGS LDFLAGS LDLIBS ZLIB_CPPFLAGS ZLIB_LDLIBS FEATURE_X COMP_DB_MJ COVERAGE_REPORT DEPFILES TEST_JOBS, \
...
$(BIN_IMGNEKO): $(OBJECTS) $(CONFIG_MK) $(BUILD_INFO_H) | check-config-date
	@mkdir -p "$(dir $@)"
	$(CC) $(COMMON_LINK_FLAGS) -o "$@" $(OBJECTS) $(COMMON_LDLIBS)

$(BIN_TEST_RUNNER): $(TEST_RUNNER_OBJECT) $(SHARED_OBJECTS) $(CONFIG_MK) $(BUILD_INFO_H) | check-config-date
	@mkdir -p "$(dir $@)"
	$(CC) $(COMMON_LINK_FLAGS) -o "$@" $(TEST_RUNNER_OBJECT) $(SHARED_OBJECTS) $(COMMON_LDLIBS)
...
$(TEST_TOOL_BINS): $(BIN_DIR)/%: $(OBJ_DIR)/testing/tools/%.o $(SHARED_OBJECTS) $(CONFIG_MK) $(BUILD_INFO_H) | check-config-date
	@mkdir -p "$(dir $@)"
	$(CC) $(COMMON_LINK_FLAGS) -o "$@" "$<" $(SHARED_OBJECTS) $(COMMON_LDLIBS)
